 ...
 *   - Computing mid as (low + high) / 2 overflows in fixed-width languages — use low + ((high - low) >> 1).
 *   - Minimising API calls is the real requirement.
 *
 * COMPLEXITY
 *   Naive:  O(n) time — call the API on every version from 1 upward.
 *   Target: O(log n) time — binary search for the boundary, keeping mid as a candidate when it is bad rather than discarding it.
 * ----------------------------------------------------------------------
 */
